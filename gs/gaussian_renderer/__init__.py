...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    if not return_plane:
        rendered_image, radii, out_observe, _ = rasterizer(
            means3D = means3D,
            means2D = means2D,
    ...
    rendered_image, radii, out_observe, out_all_map = rasterizer(
        means3D = means3D,
        means2D = means2D,
        means2D_abs = means2D_abs,
        shs = shs,
    ...
    rendered_normal2 = rendered_normal.permute(1,2,0).view(-1,3)
    plane_depth = rendered_distance.view(-1) / ((rendered_normal2 * rays_d.reshape(-1,3)).sum(-1).abs() + 1.0e-8)
    plane_depth = plane_depth.reshape(*rendered_alpha.shape)
    
    # plane_depth = rendered_distance
    return_dict =  {"render": rendered_image,
                    "viewspace_points": screenspace_points,
                    "viewspace_points_abs": screenspace_points_abs,
                    "visibility_filter" : radii > 0,
