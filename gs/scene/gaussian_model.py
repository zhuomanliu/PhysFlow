...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        print(f"box x {self._xyz[0].min()} -> {self._xyz[0].max()}, y {self._xyz[1].min()} -> {self._xyz[1].max()}, z {self._xyz[2].min()} -> {self._xyz[2].max()}")
    ...
    def load_ply(self, path, material=None):
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D, N=2):
        ...
        if selected_pts_mask.sum() + n_init_points > self.max_all_points:
            ...
            mask = (torch.max(self.get_scaling, dim=1).values > self.percent_dense*scene_extent) & (padded_max_radii2D > 10)
    ...
    def densify_and_clone(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, max_radii2D):
        n_init_points = self.get_xyz.shape[0]
        # Extract points that satisfy the gradient condition
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        # self.densify_and_clone(grads, max_grad, grads_abs, Q, extent, max_radii2D)
        # self.densify_and_split(grads, max_grad, grads_abs, Q, extent, max_radii2D)

        self.densify_and_clone(grads, max_grad, grads_abs, 1.0*max_grad, extent, max_radii2D)
        self.densify_and_split(grads, max_grad, grads_abs, 1.0*max_grad, extent, max_radii2D)
