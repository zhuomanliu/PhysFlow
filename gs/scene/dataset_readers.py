...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        if "camera_angle_x" not in contents:
            fovx = focal2fov(contents["fl_x"], contents["w"])
        else:
            fovx = contents["camera_angle_x"]
        if "fl_x" not in contents:
            if "w" not in contents:
                contents["w"] = 800
            if "h" not in contents:
                contents["h"] = 800
            contents["fl_x"] = fov2focal(fovx, contents["w"])
            contents["fl_y"] = fov2focal(fovx, contents["h"])

        frames = contents["frames"]
        for idx, frame in enumerate(frames):
            if 'jpg' or 'png' in frame["file_path"]: 
                extension = ""
            ...
            if '.png' not in cam_name and '.jpg' not in cam_name:
                cam_name += '.png'
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX,
                            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1],
                            fx=contents["fl_x"], fy=contents["fl_y"]))
